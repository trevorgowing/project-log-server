...
public class RiskDTOFactory {
    ...
    public IdentifiedRiskDTO createIdentifiedRiskDTO(Risk risk) {
        ...
        IdentifiedProjectDTO identifiedProjectDTO = projectDTOFactory.createIdentifiedProjectDTO(risk.getProject());

        return completeIdentifiedRiskDTO(risk.getId(), risk.getSummary(), risk.getDescription(),
                risk.getCategory(), risk.getImpact(), risk.getStatus(), risk.getDateClosed(), identifiedProjectDTO,
                identifiedUserDTO, risk.getProbability(), risk.getRiskResponse());
    }
}
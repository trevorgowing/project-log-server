...
public class RiskDTOFactoryTests extends AbstractTests {
  ...
  @InjectMocks private RiskDTOFactory riskDTOFactory;

  @Test
  public void testCreateIdentifiedRiskDTO_shouldCreateIdentifiedRiskDTOWithValuesFromGivenRisk() {
    // Set up fixture
    Risk risk =
        aCompleteRisk().project(aProject().id(1L).build()).owner(aUser().id(1L).build()).build();
    ...
    IdentifiedRiskDTO expectedIdentifiedRiskDTO =
        anIdentifiedRiskDTO()
            .id(risk.getId())
    ...
    when(projectDTOFactory.createIdentifiedProjectDTO(risk.getProject()))
        .thenReturn(identifiedProjectDTO);

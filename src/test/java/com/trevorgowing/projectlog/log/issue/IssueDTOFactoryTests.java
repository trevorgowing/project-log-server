...
public class IssueDTOFactoryTests extends AbstractTests {
  ...
  @InjectMocks private IssueDTOFactory issueDTOFactory;

  @Test
  public void testCreateIdentifiedIssueDTO_shouldCreateIdentifiedRiskDTOWithValuesFromGivenRisk() {
    // Set up fixtures
    Issue issue =
        aCompleteIssue().project(aProject().id(1L).build()).owner(aUser().id(1L).build()).build();
    ...
    when(projectDTOFactory.createIdentifiedProjectDTO(issue.getProject()))

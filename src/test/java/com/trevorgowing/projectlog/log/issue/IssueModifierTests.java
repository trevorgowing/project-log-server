...
public class IssueModifierTests extends AbstractTests {
  ...
  public void testUpdateIssueWithNonExistentProject_shouldThrowProjectNotFoundException() {
    ...
    IdentifiedIssueDTO identifiedIssueDTO =
        anIdentifiedIssueDTO().id(1L).project(anIdentifiedProjectDTO().id(2L).build()).build();

    Issue issue = anIssue().id(1L).project(aProject().id(1L).build()).build();
  ...
  public void testUpdateIssueWithNonExistentOwner_shouldThrowUserNotFoundException() {
    // Set up fixture
    IdentifiedIssueDTO identifiedIssueDTO =
        anIdentifiedIssueDTO().id(1L).owner(anIdentifiedUserDTO().id(2L).build()).build();

    Issue issue = anIssue().id(1L).owner(aUser().id(1L).build()).build();
  ...
  @Test
  public void
      testUpdateIssueWithValidIssue_shouldDelegateToIssueRepositoryToSaveIssueAndReturnUpdateIssue() {
    // Set up expectations
    IdentifiedIssueDTO identifiedIssueDTO =
    ...
    when(issueRepository.save(argThat(samePropertyValuesAs(expectedIssue))))
